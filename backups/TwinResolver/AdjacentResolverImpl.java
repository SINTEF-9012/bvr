...
public class AdjacentResolverImpl implements AdjacentResolver {
	...
	public void resolve(FragSubHolder fragmentHolderCurrent) throws BasicCVLEngineException {
		...
		logger.debug("adjacent resolution for the fragment(current): " + fragmentHolderCurrent.getFragment() + " with placement: " + fragmentHolderCurrent.getPlacement().getPlacementFragment());
		logger.debug("found adjacent fragments: " + adjacentFragments);
		debugPrintoutAdjacentFrgaments(adjacentFragments);
		logger.debug("found twins fragments: " + twinAFrag);
		debugPrintoutAdjacentFrgaments(twinAFrag);
		
		for(AdjacentFragment adjacentFragment : adjacentFragments){
			
			logger.debug("processing adjacent fragment: " + adjacentFragment + " with placement: " + adjacentFragment.getFragmentHolder().getPlacement().getPlacementFragment());
			...
			for(Map.Entry<FromBinding, ToBinding> entry : adjacentBindingsToCurrent.entrySet()){
				...
				for(ObjectHandle objectHandle : insideBoundaryElementsFromPlacement){
					...
					logger.debug("inside neighbouring element (object: " + insideBoundaryElementPlc + ", property: "+propertyName+", upperBound: " +upperBound + ") " +
					" of the adjacent fragment should reference these elements: " + insideBOHElmtsPlcReplaced);
					...
					if(upperBound == -1 || upperBound > 1){
						...
						if(insideBOHElmtsPlcReplaced.size() != upperBound){
							if(twinAFrag.isEmpty())
								throw new IllegalCVLOperation("cardinality does not match for property : '" +
										propertyName + "' (upperBound "+ upperBound +", " + insideBOHElmtsPlcReplaced +
										", "+ EngineUtility.resolveProxies(insideBOHElmtsPlcReplaced)+") of the fragment " +
										fragHolderAdjacent.getFragment());
							logger.warn("cardinality does not match for property : '" +
									propertyName + "' (upperBound "+ upperBound +", " + insideBOHElmtsPlcReplaced +
									", "+ EngineUtility.resolveProxies(insideBOHElmtsPlcReplaced)+") of the fragment " +
									fragHolderAdjacent.getFragment());
							insideBOHElmtsPlcReplaced.clear();
							insideBOHElmtsPlcReplaced.addAll(toBinding.getToPlacement().getInsideBoundaryElement());
							logger.debug("!!!!!!!!!!!!!!!!!!!!!!!!! " + EngineUtility.resolveProxies(insideBOHElmtsPlcReplaced));
						}
	...
	private void debugPrintoutAdjacentFrgaments(Collection<AdjacentFragment> adjacentFragments){
		for(AdjacentFragment fragment : adjacentFragments){
			logger.debug(fragment + "/" + fragment.getFragmentHolder().getFragment() + "/" + fragment.getFragmentHolder().getPlacement().getPlacementFragment());
		}
	}
}
