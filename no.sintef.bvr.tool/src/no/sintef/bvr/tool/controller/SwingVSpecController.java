...
public class SwingVSpecController<GUI_NODE extends JComponent, MODEL_OBJECT extends EObject, SERIALIZABLE extends Serializable> implements
		VSpecControllerInterface<GUI_NODE, MODEL_OBJECT, SERIALIZABLE> {
	...
	protected void loadBVRView(BVRModel model, BVRUIKernel<BVREditorPanel, BVRModelPanel> uikernel) throws BVRModelException {
		...
		JComponent rootComponent = new AddBVRModel<BVREditorPanel, BVRModelPanel>().init(uikernel, model, null, vspecvmMap, vspecNodes, vspecBindings,
	...
	protected void loadBVRView(VSpec v, BVRUIKernel<BVREditorPanel, BVRModelPanel> kernel, JComponent parent, BVRModel model) throws BVRModelException {
		...
		if (v instanceof VClassifier) {
			JComponent c = new AddVClassifier<BVREditorPanel, BVRModelPanel>(toolModel.isVSpecMinimized(v)).init(kernel, v, parent, vspecvmMap, vspecNodes,
					vspecBindings, rootController).execute();
			nextParent = c;
		} else if (v instanceof Choice) {
			JComponent c = new AddChoice<BVREditorPanel, BVRModelPanel>(toolModel.isVSpecMinimized(v)).init(kernel, v, parent, vspecvmMap, vspecNodes,
					vspecBindings, rootController).execute();
			nextParent = c;
		} else if (v instanceof ChoiceOccurrence) {
			JComponent c = new AddChoiceOccurence<BVREditorPanel, BVRModelPanel>(toolModel.isVSpecMinimized(v)).init(kernel, v, parent, vspecvmMap, vspecNodes,
					vspecBindings, rootController).execute();
			nextParent = c;
		} else if (v instanceof VClassOccurrence) {
			JComponent c = new AddVClassOccurence<BVREditorPanel, BVRModelPanel>(toolModel.isVSpecMinimized(v)).init(kernel, v, parent, vspecvmMap, vspecNodes,
					vspecBindings, rootController).execute();
			nextParent = c;
		}

		if ((v instanceof VSpec) && !toolModel.isVSpecMinimized(v)) {
			for (Constraint c : ((VNode) v).getOwnedConstraint()) {
				if (c instanceof BCLConstraint) {
					BCLConstraint bcl = (BCLConstraint) c;
					new AddBCLConstraint<BVREditorPanel, BVRModelPanel>().init(kernel, bcl, nextParent, vspecvmMap, vspecNodes, vspecBindings, rootController)
							.execute();
				}
			}
			...
			if (v instanceof CompoundNode) {
				if (((CompoundNode) v).getGroupMultiplicity() != null)
					nextParent = new AddGroupMultiplicity<BVREditorPanel, BVRModelPanel>().init(kernel, v, nextParent, vspecvmMap, vspecNodes, vspecBindings,
							rootController).execute();
	...
	public void addChoice(GUI_NODE parent) {
		final NamedElement parentNamedElement = vspecvmMap.get(parent);
		SimpleExeCommandBatchDecorator command = new SimpleExeCommandBatchDecorator(new SimpleExeCommandInterface() {
	...
	public Command<?, ?> createUpdateChoiceCommand(GUI_NODE node) {
		Command<BVREditorPanel, BVRModelPanel> command = new UpdateChoiceBatchCommandDecorator<BVREditorPanel, BVRModelPanel>(
				new UpdateChoice<BVREditorPanel, BVRModelPanel>());
	...
	public void setNodeComment(GUI_NODE node, String comment) {
		NamedElement namedElement = vspecvmMap.get(node);
		toolModel.updateComment(namedElement, comment);
	}

	@Override
	public void setNodeName(GUI_NODE node, String name) {
		final NamedElement namedElement = vspecvmMap.get(node);
		final String name_update = name;
		SimpleExeCommandBatchDecorator command = new SimpleExeCommandBatchDecorator(new SimpleExeCommandInterface() {

			@Override
			public void execute() {
				toolModel.updateName(namedElement, name_update);
			}
		});
		command.execute();
	}
	...
	public Command<?, ?> createUpdateVClassifierCommand(GUI_NODE node) {
		Command<BVREditorPanel, BVRModelPanel> command = new UpdateVClassifierBatchCommandDecorator<BVREditorPanel, BVRModelPanel>(
				new UpdateVClassifier<BVREditorPanel, BVRModelPanel>());
		command.init(vSpecbvruikernel, vspecvmMap.get(node), node, vspecvmMap, vspecNodes, vspecBindings, rootController);
		return command;
	}
	...
	public void addVClassifier(GUI_NODE node) {
		final NamedElement parentNamedElement = vspecvmMap.get(node);
		SimpleExeCommandBatchDecorator command = new SimpleExeCommandBatchDecorator(new SimpleExeCommandInterface() {
	...
	public Command<?, ?> createUpdateBCLConstraintCommand(GUI_NODE node) {
		Command<BVREditorPanel, BVRModelPanel> command = new UpdateConstraintBatchCommandDecorator<BVREditorPanel, BVRModelPanel>(
				new UpdateBCLConstraint<BVREditorPanel, BVRModelPanel>());
		command.init(vSpecbvruikernel, vspecvmMap.get(node), node, vspecvmMap, vspecNodes, vspecBindings, rootController);
		return command;
	}
	...
	public void removeNamedElement(GUI_NODE node) {
		NamedElement element = vspecvmMap.get(node);
	...
	public void exportAsPNGImage(SERIALIZABLE _file) {
		...
		JLayeredPane layeredPanel = vSpecbvruikernel.getModelPanel();
	...
	public Command createUpdateChoiceOccurenceCommand(GUI_NODE node) {
		Command<BVREditorPanel, BVRModelPanel> command = new UpdateChoiceOccurenceBatchCommandDecorator<BVREditorPanel, BVRModelPanel>(
				new UpdateChoiceOccurence<BVREditorPanel, BVRModelPanel>());
		command.init(vSpecbvruikernel, vspecvmMap.get(node), node, vspecvmMap, vspecNodes, vspecBindings, rootController);
		return command;
	}
	...
	public Command createUpdateVClassOccurenceCommand(GUI_NODE elem) {
		Command<BVREditorPanel, BVRModelPanel> command = new UpdateVClassOccurenceBatchCommandDecorator<BVREditorPanel, BVRModelPanel>(
				new UpdateVClassOccurence<BVREditorPanel, BVRModelPanel>());
	...
	@Override
	public void setVClassOccurenceGroupMultiplicityUpperBound(GUI_NODE node, int upperBound) {
	...
	@Override
	public void setVClassOccurenceGroupMultiplicityLowerBound(GUI_NODE node, int lowerBound) {
		VClassOccurrence vClassOccur = (VClassOccurrence) vspecvmMap.get(node);
