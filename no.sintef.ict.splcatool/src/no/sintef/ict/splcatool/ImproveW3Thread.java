...
public class ImproveW3Thread implements Runnable, ProgressReporter {
	...
	ImproveW3Thread(CNF cnf, int t, CoveringArray ca,
	...
	public void run() {
		try {
			improve_weighted_3(cnf, t, ca, fmFileName, threads, weightFileName, orgCov, l);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	
	private void improve_weighted_3(CNF cnf, int t, CoveringArray ca, String fmFileName,
			int threads, String weightFileName, double orgCov, List<Improvement> l)
		...
		for(int i = 0; i < caf.rows.size(); i++){
			for(int j = begin; j < end; j++){
				for(int k = j+1; k < caf.rows.get(i).size(); k++){
					for(int m = k+1; m < caf.rows.get(i).size(); m++){
					// Change first
					...
					if(isValid){
						...
						// store improvement
						if(newCov > orgCov){
							Improvement imp = new Improvement3(newCov, i, caf.getId(j+1), caf.getId(k+1), caf.getId(m+1), (x==0?'X':'-'), (y==0?'X':'-'), (z==0?'X':'-'), (newCov-orgCov)/orgCov);
							l.add(imp);
						}
