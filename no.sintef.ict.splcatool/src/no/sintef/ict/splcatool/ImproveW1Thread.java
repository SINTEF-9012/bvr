...
public class ImproveW1Thread implements Runnable, ProgressReporter {
	...
	ImproveW1Thread(CNF cnf, int t, CoveringArray ca,
	...
	public void run() {
		try {
			improve_weighted_1(cnf, t, ca, fmFileName, threads, weightFileName, orgCov, l);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	
	private void improve_weighted_1(CNF cnf, int t, CoveringArray ca,
			String fmFileName, int threads, String weightFileName,
			double orgCov, List<Improvement> l)
		...
		for(int i = 0; i < caf.rows.size(); i++){
			for(int j = begin; j < end; j++){
				...
				if(isValid){
					...
					// store improvement
					if(newCov > orgCov){
						Improvement imp = new Improvement(newCov, i, caf.getId(j+1), (x==0?'X':'-'), (newCov-orgCov)/orgCov);
						l.add(imp);
					}
